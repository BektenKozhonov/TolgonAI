...
load_dotenv('D:/bot_voice_recognition/.env')
...
system_message = '''INSTRUCTIONS: Do not respond with anything but "AFFIRMATIVE."
                    ...
                    Task:
                    Identify Key Points:
                    Extract the main ideas from the provided text.
                    Focus on crucial information and important highlights.
                    Summarize Concisely:
                    Convert the extracted key points into a shorter format.
                    Maintain clarity and coherence while reducing the text length.
                    Handle Noisy Data:
                    Be aware that the text may contain interferences and noise.
                    Ensure the summary remains meaningful and accurate despite potential distortions in the input.'''
...
def convert_ogg_to_wav():
    song = AudioSegment.from_ogg('D:/bot_voice_recognition/audio.ogg')
    song.export('D:/bot_voice_recognition/audio.wav', format="wav")

def listen_for_wav_to_text():
    convert_ogg_to_wav()
    text = 'Words not recognized.'
    file = 'D:/bot_voice_recognition/audio.wav'
...
def _clear():
    _files = ['D:/bot_voice_recognition/audio.wav', 'D:/bot_voice_recognition/audio.ogg']
    for _file in _files:
...
def voice_handler(message):
    ...
    if int(file_size) >= 715000:
        ...
        with open('D:/bot_voice_recognition/audio.ogg', 'wb') as f:
